...
def encode(encoder, bpp_target, image, width, height, pix_fmt, depth):
    ...
    output_dir = os.path.join('./output_hevc/' + encoder_name)
    mkdir_p(output_dir)
    ...
    if os.path.isfile(image_out):
        ...
        return image_out
    encode_script = os.path.join('./encode_hevc/', encoder)
...
def decode(decoder, encoded_image, width, height, pix_fmt, depth):
    ...
    output_dir = os.path.join('./output_hevc/', decoder_name, 'decoded')
    mkdir_p(output_dir)

    decode_script = os.path.join('./decode_hevc/', decoder)
    if pix_fmt == "ppm":
        ext_name = '.ppm'
    ...
    if not os.path.isfile(ppm_dest):
        try:
            ...
            cmd = ["/tools/difftest_ng-master/difftest_ng", "--crop", "0", "0", cropwidth, cropheight,
                   "--convert", ppm_dest, os.path.join(image), "-"]
            subprocess.check_output(" ".join(cmd), stderr=subprocess.STDOUT, shell=True)
        except subprocess.CalledProcessError as e:
            print cmd, e.output
            raise e
            ...
            if not os.path.isfile(yuv_dest):
                try:
                    ...
                    if depth == '32':
                        ...
                        cmd = ["/tools/difftest_ng-master/difftest_ng", "--crop", "0", "0", cropwidth, cropheight,
                               "--toycbcr", "--csub 2 2",
                               "--convert", yuv_output, os.path.join(image), '-']
                    subprocess.check_output(" ".join(cmd), stderr=subprocess.STDOUT, shell=True)
                except subprocess.CalledProcessError as e:
                    print cmd, e.output
                    raise e
...
def main():
    ...
    encoders = set(os.listdir('encode_hevc'))
    decoders = set(os.listdir('decode_hevc'))
